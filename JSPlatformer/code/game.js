...
function Broc(pos, ch) {
  this.pos = pos;
  this.size = new Vector(2, 1.5);
  if (ch == "^") {
    // Horizontal lava
    this.speed = new Vector(2, 0);
  } 
...
Broc.prototype.act = function(step, level) {
  var newPos = this.pos.plus(this.speed.times(step));
  if (!level.obstacleAt(newPos, this.size))
    this.pos = newPos;
  else if (this.repeatPos)
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  else if (type == "broc" && this.status == null) {
    this.status = "lost";
    this.finishDelay = 1;
  } 
  else if (type == "coin") {
...
function runLevel(level, Display, andThen) {
  var display = new Display(document.body, level);
	alert("Welcome to Kandy Kingdom! Collect candy for a super jump. Watch out for broccoli and melted chocolate!");
